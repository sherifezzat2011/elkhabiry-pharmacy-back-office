...
function MapPointSelector({ points, onChange }: { points: { x: number; y: number }[]; onChange: (points: { x: number; y: number }[]) => void }) {
  ...
  return <div className="rounded-lg border border-brand-100 bg-brand-50/40 p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-bold text-slate-800">Optional Map Points</p><p className="mt-1 text-xs text-slate-500">Click at least 3 points to draw a custom zone boundary.</p></div>{points.length ? <Button size="sm" onClick={() => onChange([])}>Clear Points</Button> : null}</div><button type="button" className="relative mt-3 h-48 w-full overflow-hidden rounded-lg border border-slate-200 bg-white text-left" onClick={addPoint}><div className="absolute inset-0 bg-[linear-gradient(90deg,#e2e8f0_1px,transparent_1px),linear-gradient(#e2e8f0_1px,transparent_1px)] bg-[size:36px_36px]" />{points.length >= 3 ? <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none"><polygon points={points.map((point) => `${point.x},${point.y}`).join(" ")} className="fill-brand-100/70 stroke-brand-500" strokeWidth="1.5" /></svg> : null}{points.map((point, index) => <span key={`${point.x}-${point.y}-${index}`} className="absolute grid h-7 w-7 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-brand-600 text-xs font-bold text-white shadow-soft" style={{ left: `${point.x}%`, top: `${point.y}%` }}>{index + 1}</span>)}<span className="absolute bottom-3 left-3 rounded-md bg-white/95 px-2.5 py-1 text-xs font-semibold text-slate-600 shadow-sm">{points.length ? `${points.length} point${points.length === 1 ? "" : "s"} selected` : "Click on the map to add points"}</span></button></div>;
}
