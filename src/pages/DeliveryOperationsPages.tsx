...
type ZonesChangeHandler = (zones: DeliveryZone[]) => void;
...
function MapPointSelector({ points, onChange }: { points: { lat: number; lng: number }[]; onChange: (points: { lat: number; lng: number }[]) => void }) {
  ...
  const addPoint = (point: { lat: number; lng: number }) => onChange([...points, point]);
  const removePoint = (index: number) => onChange(points.filter((_, pointIndex) => pointIndex !== index));
  return <div className="rounded-lg border border-brand-100 bg-brand-50/40 p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-bold text-slate-800">Optional Map Points</p><p className="mt-1 text-xs text-slate-500">Click the map to open a larger workspace, then add at least 3 points. Click a selected point to remove it.</p></div><div className="flex shrink-0 gap-2"><Button size="sm" onClick={() => setMapOpen(true)}>Open Large Map</Button>{points.length ? <Button size="sm" onClick={() => onChange([])}>Clear Points</Button> : null}</div></div><RealMapSurface points={points} onAddPoint={addPoint} onRemovePoint={removePoint} onRequestExpand={() => setMapOpen(true)} expandOnMapClick />{mapOpen ? <div className="fixed inset-0 z-[70] bg-slate-950/80 p-3 backdrop-blur-sm sm:p-5"><div className="flex h-full flex-col overflow-hidden rounded-xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-100 px-4 py-3"><div><p className="text-base font-bold text-slate-950">Delivery Zone Map</p><p className="text-xs text-slate-500">Search, zoom, drag, click to add points, or click a point to remove it.</p></div><Button variant="ghost" size="icon" onClick={() => setMapOpen(false)}><X className="h-5 w-5" /></Button></div><div className="min-h-0 flex-1 p-3"><RealMapSurface points={points} onAddPoint={addPoint} onRemovePoint={removePoint} heightClass="h-full" /></div><div className="flex items-center justify-between border-t border-slate-100 bg-slate-50 px-4 py-3 text-xs font-semibold text-slate-600"><span>{points.length} point{points.length === 1 ? "" : "s"} selected</span><Button size="sm" variant="primary" onClick={() => setMapOpen(false)}>Done</Button></div></div></div> : null}</div>;
}

function RealMapSurface({ points, onAddPoint, onRemovePoint, onRequestExpand, expandOnMapClick = false, readonly = false, heightClass = "h-80" }: { points: { lat: number; lng: number }[]; onAddPoint?: (point: { lat: number; lng: number }) => void; onRemovePoint?: (index: number) => void; onRequestExpand?: () => void; expandOnMapClick?: boolean; readonly?: boolean; heightClass?: string }) {
  ...
  const addPoint = (event: MouseEvent<HTMLDivElement>) => {
    ...
    if (expandOnMapClick && onRequestExpand) {
      onRequestExpand();
      return;
    }
  ...
  const endDrag = () => {
    suppressNextClickRef.current = Boolean(dragRef.current?.moved);
  ...
  return <div ref={mapRef} className={cn("relative mt-3 w-full overflow-hidden rounded-lg border border-slate-200 bg-slate-200 shadow-inner", heightClass)} onClick={addPoint} onMouseDown={startDrag} onMouseMove={drag} onMouseLeave={endDrag} onMouseUp={endDrag}>{tiles.map((tile) => <img key={`${tile.x}-${tile.y}-${zoom}`} className="absolute select-none" alt="" draggable={false} src={`https://tile.openstreetmap.org/${zoom}/${tile.wrappedX}/${tile.y}.png`} style={{ left: tile.left, top: tile.top, width: tileSize, height: tileSize }} />)}<svg className="pointer-events-none absolute inset-0 z-10 h-full w-full">{screenPoints.length >= 3 ? <polygon points={screenPoints.map((point) => `${point.x},${point.y}`).join(" ")} className="fill-brand-100/60 stroke-brand-500" strokeWidth="3" /> : null}</svg>{screenPoints.map((point, index) => readonly ? <span key={`${point.lat}-${point.lng}-${index}`} className="pointer-events-none absolute z-20 grid h-7 w-7 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-brand-600 text-xs font-bold text-white shadow-soft ring-2 ring-white" style={{ left: point.x, top: point.y }}>{index + 1}</span> : <button key={`${point.lat}-${point.lng}-${index}`} type="button" title={`Remove point ${index + 1}`} aria-label={`Remove point ${index + 1}`} className="absolute z-20 grid h-7 w-7 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-brand-600 text-xs font-bold text-white shadow-soft ring-2 ring-white transition hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-200" style={{ left: point.x, top: point.y }} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); onRemovePoint?.(index); }}>{index + 1}</button>)}{onRequestExpand ? <button type="button" className="absolute left-3 bottom-12 z-30 rounded-md bg-white/95 px-3 py-1.5 text-xs font-bold text-brand-700 shadow-sm hover:bg-brand-50" onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); onRequestExpand(); }}>Expand Map</button> : null}{!readonly ? <form className="absolute left-3 top-3 z-30 w-[min(340px,calc(100%-6rem))]" onSubmit={searchPlace} onClick={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()}><div className="flex h-10 items-center gap-2 rounded-md bg-white px-3 text-sm text-slate-500 shadow-sm"><Search className="h-4 w-4 shrink-0" /><input className="min-w-0 flex-1 bg-transparent text-slate-900 outline-none placeholder:text-slate-500" placeholder="Search map" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} /><button type="submit" className="rounded px-2 py-1 text-xs font-bold text-brand-700 hover:bg-brand-50" disabled={searching}>{searching ? "..." : "Go"}</button></div>{searchResults.length || searchError ? <div className="mt-2 max-h-44 overflow-y-auto rounded-md bg-white py-1 text-xs shadow-soft ring-1 ring-slate-200">{searchResults.map((result) => <button key={result.place_id} type="button" className="block w-full px-3 py-2 text-left leading-5 text-slate-700 hover:bg-brand-50 hover:text-brand-900" onClick={() => selectSearchResult(result)}>{result.display_name}</button>)}{searchError ? <p className="px-3 py-2 font-semibold text-slate-500">{searchError}</p> : null}</div> : null}</form> : null}<div className="absolute right-3 top-3 z-20 overflow-hidden rounded-md bg-white text-slate-700 shadow-sm" onMouseDown={(event) => event.stopPropagation()}><button type="button" className="grid h-9 w-9 place-items-center border-b border-slate-200 text-2xl font-bold" onClick={(event) => { event.stopPropagation(); zoomBy(1); }}>+</button><button type="button" className="grid h-9 w-9 place-items-center border-b border-slate-200 text-2xl font-bold" onClick={(event) => { event.stopPropagation(); zoomBy(-1); }}>-</button><span className="grid h-9 w-9 place-items-center text-xs font-bold">{zoom}</span></div><button type="button" className="absolute right-3 top-[122px] z-20 grid h-9 w-9 place-items-center rounded-md bg-white text-xs font-bold text-slate-700 shadow-sm" onMouseDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); setCenter(defaultMapCenter); setZoom(12); }}>Fit</button><span className="absolute bottom-3 left-3 z-20 rounded-md bg-white/95 px-2.5 py-1 text-xs font-bold text-slate-700 shadow-sm">{points.length ? `${points.length} point${points.length === 1 ? "" : "s"} selected` : readonly ? "No custom boundary points" : expandOnMapClick ? "Click to open large map" : "Click on the map to add points"}</span><span className="absolute bottom-3 right-3 z-20 rounded bg-white/85 px-2 py-1 text-[10px] font-semibold text-brand-700">(c) OpenStreetMap contributors</span></div>;
}
...
export function DeliveryZonesPage({ initialZones, initialChannels, onZonesChange }: { initialZones: DeliveryZone[]; initialChannels: DeliveryChannel[]; onZonesChange?: ZonesChangeHandler }) {
...
export function DeliveryPricingPage({ initialZones, onZonesChange }: { initialZones: DeliveryZone[]; onZonesChange?: ZonesChangeHandler }) {
  ...
  return <div className="mx-auto max-w-[1680px] space-y-5"><Hero title="Delivery Pricing" subtitle="Manage delivery fee priority using zone fees, distance rules, and the default fee." /><section className="grid gap-4 md:grid-cols-3">{rules.map(([name, fee, detail]) => <Card key={name} className="p-5"><p className="text-sm font-bold text-slate-500">{name}</p><p className="mt-3 text-2xl font-bold text-brand-700">{fee}</p><p className="mt-2 text-sm leading-6 text-slate-500">{detail}</p></Card>)}</section><Card className="overflow-hidden"><div className="overflow-x-auto scrollbar-soft"><table className="w-full min-w-[860px] text-sm"><thead className="bg-brand-50 text-brand-900"><tr>{["Zone", "Applied Fee", "Priority", "Status"].map((header) => <th key={header} className="h-12 px-4 text-center align-middle text-xs font-bold uppercase tracking-wide">{header}</th>)}</tr></thead><tbody className="divide-y divide-slate-100 bg-white">{zones.map((zone) => <tr key={zone.id} className="transition hover:bg-brand-50/40"><td className="px-4 py-4 text-center font-bold text-slate-900">{zone.name}</td><td className="px-4 py-4 text-center font-medium text-slate-700"><div className="mx-auto flex max-w-36 items-center gap-2"><input className={`${inputClass} w-24 text-center`} type="number" min={0} value={zone.deliveryFee} onChange={(event) => updateFee(zone.id, Number(event.target.value))} /><span className="text-xs font-bold text-slate-500">EGP</span></div></td><td className="px-4 py-4 text-center font-medium text-slate-700">1. Zone-specific delivery fee</td><td className="px-4 py-4 text-center"><Badge value={zone.status} /></td></tr>)}</tbody></table></div></Card></div>;
}
