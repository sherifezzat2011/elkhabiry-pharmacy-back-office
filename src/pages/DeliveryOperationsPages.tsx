...
function MapPointSelector({ points, onChange }: { points: { lat: number; lng: number }[]; onChange: (points: { lat: number; lng: number }[]) => void }) {
  return <div className="rounded-lg border border-brand-100 bg-brand-50/40 p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-bold text-slate-800">Optional Map Points</p><p className="mt-1 text-xs text-slate-500">Zoom or drag the real map, then click at least 3 points to draw a custom zone boundary. Click a selected point to remove it.</p></div>{points.length ? <Button size="sm" onClick={() => onChange([])}>Clear Points</Button> : null}</div><RealMapSurface points={points} onAddPoint={(point) => onChange([...points, point])} onRemovePoint={(index) => onChange(points.filter((_, pointIndex) => pointIndex !== index))} /></div>;
}

function RealMapSurface({ points, onAddPoint, onRemovePoint, readonly = false, heightClass = "h-80" }: { points: { lat: number; lng: number }[]; onAddPoint?: (point: { lat: number; lng: number }) => void; onRemovePoint?: (index: number) => void; readonly?: boolean; heightClass?: string }) {
