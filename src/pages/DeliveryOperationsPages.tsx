...
import { Eye, MapPinned, MoreHorizontal, Pencil, Plus, Search, X } from "lucide-react";
...
function ActionMenu({ status, onToggle, onDelete }: { status: DeliveryStatus; onToggle: () => void; onDelete: () => void }) {
  return <details className="relative"><summary className="mx-auto grid h-10 w-10 cursor-pointer list-none place-items-center rounded-lg border border-transparent text-slate-600 transition hover:bg-brand-50 hover:text-brand-800" aria-label="More Actions" title="More Actions"><MoreHorizontal className="h-4 w-4" /></summary><div className="absolute right-0 z-20 mt-2 w-44 rounded-lg border border-slate-200 bg-white p-1.5 text-sm shadow-soft"><button className="w-full rounded-md px-3 py-2 text-left hover:bg-brand-50" onClick={onToggle}>{status === "Active" ? "Deactivate" : "Activate"}</button><button className="w-full rounded-md px-3 py-2 text-left text-red-700 hover:bg-red-50" onClick={onDelete}>Delete</button></div></details>;
}
